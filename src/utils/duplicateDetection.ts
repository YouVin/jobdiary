...
export function getImportDecision(
  ...
  // 1) externalId가 있으면(사람인/잡코리아) 같은 externalId의 기존 건이 있는지 확인 → 있으면 기술적 중복(재수집)이라 스킵
  if (newApplication.externalId) {
    const hasSameExternalId = existingApplications.some(
      (existing) => existing.externalId === newApplication.externalId,
    );
    ...
    // 2) externalId가 없으면(원티드) company + position + appliedAt(날짜)까지 전부 같은 기존 건을

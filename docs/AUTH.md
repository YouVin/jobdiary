...
| `/account` | 계정 관리(비밀번호/이메일 변경, 탈퇴, 프로필) | 로그인 필수 | 기존 `AuthGuard`로 그대로 감쌀 수 있음 — 비밀번호/이메일 변경/탈퇴의 구체적 설계는 §6 Phase 5(§6.5.1~§6.5.7)에서 확정 |
...
- [ ] **`service_role` 키**: 회원 탈퇴 기능(§2.6) 구현 시에도 클라이언트 번들에 절대 포함되지 않도록 — Supabase Edge Function 환경변수(`SUPABASE_SERVICE_ROLE_KEY`, Supabase가 자동 주입)로만 존재해야 한다. 상세 설계는 §6 Phase 5(§6.5.4).
- [ ] **`applications` 테이블의 `user_id` FK ON DELETE 동작**: `auth.users` 삭제 시 연관 `applications` 행이 자동으로 함께 삭제(`CASCADE`)되는지 실제 스키마에서 확인 — 아니라면 회원 탈퇴 Edge Function이 직접 정리하거나 마이그레이션으로 `CASCADE`를 추가해야 한다(§6.5.4).
...
### Phase 5 — 계정 관리
- 범위(이번 상세 설계): `/account` 페이지, 비밀번호 변경, 이메일 변경, 회원 탈퇴. **프로필은 이번 상세 설계에서 제외** — 스키마 결정(§2.6)이 먼저 필요해 범위를 분리했다.
- 의존성: 회원 탈퇴는 이 프로젝트에 아직 없는 서버리스 함수 인프라(Supabase Edge Function)가 새로 필요하다 — 비밀번호/이메일 변경과 별도로 인프라 준비부터 시작해야 한다(§6.5.4).
- 기존 구조 영향: `/account` 자체는 `AuthGuard`를 그대로 재사용해 신규 페이지만 추가하면 된다 — `authStore`/리다이렉트 소유권 구조는 바뀌지 않는다. 탈퇴 완료 후 로그아웃 처리만 기존 `signOut()`을 재사용한다.

#### 6.5.1 계정 페이지 구조

- 경로: `/account`. `AuthGuard`로 감싼다(로그인 필수) — 별도 가드 로직이 필요했던 `/update-password`(§6.3.4)와 달리 `/account`는 "인증됨=허용"이 그대로 맞으므로 기존 `AuthGuard`를 수정 없이 재사용한다.
- 섹션 3개, 위험도 순으로 배치:
  1. 비밀번호 변경(§6.5.2)
  2. 이메일 변경(§6.5.3)
  3. 회원 탈퇴 — "위험 구역"으로 시각적으로 분리(예: `status-rejected` 톤의 테두리/배경으로 다른 두 섹션과 구분, 페이지 맨 아래 배치) — 실수로 다른 설정과 헷갈려 누르지 않도록.
- 진입점: `HeaderActions`(`src/components/header/HeaderActions.tsx`)에 버튼을 하나 추가한다. 현재 버튼들(복사/공유/지원추가/로그아웃)과 같은 줄, 로그아웃 버튼 바로 앞에 "계정 설정" 버튼을 추가하고 `/account`로 이동시킨다 — 기존 버튼과 같은 보더 스타일(`border-card-border`)을 그대로 써서 새 컴포넌트를 만들지 않고 자연스럽게 끼워 넣는다. 드롭다운 메뉴 같은 새 UI 패턴은 도입하지 않는다(범위 최소화).

#### 6.5.2 비밀번호 변경

- 호출: `supabase.auth.updateUser({ password })`.
- **현재 비밀번호 재확인 — 확정 제안: 요구한다.** `updateUser`는 API 차원에서 현재 비밀번호를 요구하지 않는다(이미 유효한 세션이 있으면 그것으로 충분하다고 간주한다). 하지만 세션이 탈취되거나(XSS, 공용 PC에 로그인 상태로 방치 등) 잠깐 자리를 비운 사이 제3자가 접근하면, 재확인 없이 비밀번호를 그대로 바꿔 계정을 탈취/잠금할 수 있다. 그래서 새 비밀번호를 받기 전에 **현재 비밀번호를 한 번 더 입력받아 확인**한다.
  - Supabase SDK엔 "비밀번호만 검증"하는 전용 API가 없으므로, 현재 로그인 이메일 + 입력받은 현재 비밀번호로 `supabase.auth.signInWithPassword(...)`를 다시 호출해 성공 여부로 검증한다(재로그인처럼 보이지만 실제로는 "확인" 용도). 성공해야만 이어서 `updateUser({ password: newPassword })`를 호출한다.
  - 이 재확인은 **이메일/비밀번호(email) identity가 있는 계정에만** 적용한다 — 구글 전용 계정에는 확인할 기존 비밀번호가 없다(§6.5.7 엣지케이스 3).
- 새 비밀번호 입력 UI는 Phase 1의 `PasswordStrengthMeter`/`getConfirmPasswordError`/`PasswordToggleButton`을 그대로 재사용(새 비밀번호 + 새 비밀번호 확인 2필드, 강도 표시).
- 성공 후: 폼을 초기화하고 "비밀번호가 변경됐습니다" 안내만 표시(페이지 이동 없음 — 이미 `/account`에 있고 세션은 그대로 유효하다). 로그아웃을 강제하지 않는다 — §6.3.2의 "재설정 후 로그아웃"은 **복구 세션**(누가 열었을지 알 수 없는 이메일 링크발 세션)에 대한 조치였지만, 여기서는 이미 현재 비밀번호로 재확인까지 마친 "확실히 본인" 세션이라 성격이 다르다.

#### 6.5.3 이메일 변경

- 호출: `supabase.auth.updateUser({ email: newEmail })`.
- **"Secure email change" 설정에 따라 흐름이 달라진다**(§5 체크리스트):
  - 켜져 있으면(권장): 기존 이메일·새 이메일 **양쪽 모두**에 확인 메일이 발송되고, 둘 다 확인해야 변경이 완료된다.
  - 꺼져 있으면: 새 이메일 쪽 확인만으로 변경된다.
  - 착수 시점에 실제 대시보드 설정값을 확인해 안내 문구를 그에 맞게 쓴다(예: "새 이메일과 기존 이메일 양쪽에서..." vs "새 이메일에서...").
- 흐름: `/account`에서 새 이메일 입력·제출 → `updateUser({ email })` 호출 → **이 시점엔 아직 이메일이 바뀌지 않는다** → "확인 대기 중" 안내로 전환 → 사용자가 메일함에서 확인 링크 클릭 → Supabase가 서버 쪽에서 `auth.users.email`을 갱신 → 클라이언트는 이미 있는 세션 갱신 경로(`onAuthStateChange` → 기존 `setSession`)를 통해 새 이메일을 반영한다. **비밀번호 재설정과 달리 새로운 `authStore` 상태가 필요 없다** — 확인 링크를 열어도 "임시 세션"이 새로 발급되는 게 아니라 기존 로그인 세션의 이메일 필드가 갱신되는 것뿐이므로, `recovering`류의 특수 처리가 필요 없다.
- 확인 링크의 redirect 목적지: 별도 페이지를 만들 필요 없이 `/account`(또는 Site URL 루트)로 지정한다 — 확인 후 사용자는 이미 로그인된 상태이므로 그냥 앱으로 돌아오면 된다.
- 변경 대기 중임을 계속 보여줄 방법(참고, 확정 아님): 세션의 `user` 객체에 "대기 중인 새 이메일"이 안정적으로 노출되는지는 supabase-js 버전에 따라 달라질 수 있어 확인이 필요하다 — 확실한 대안은 제출 성공 시 그 값을 로컬 상태로 기억해두고 "OO으로 변경 확인 대기 중"이라고 세션 동안만 보여주는 것이다(새로고침하면 사라짐, 큰 문제는 아님).

#### 6.5.4 회원 탈퇴 (서버리스, 핵심)

**문제**: 계정 삭제(`supabase.auth.admin.deleteUser`)는 관리자 API라 `service_role` 키가 필요하다. `service_role`은 RLS를 완전히 우회하는 키라 클라이언트 번들에 절대 포함될 수 없다(탈취되면 전체 사용자 데이터가 노출된다) — 이 프로젝트 전반에서 지켜온 원칙 그대로다.

**해결(확정): Supabase Edge Function.**

Vercel Function도 기술적으로는 가능하지만 Edge Function을 확정하는 이유:
- `SUPABASE_SERVICE_ROLE_KEY`가 Supabase에 의해 Edge Function 실행 환경에 **자동으로 주입**된다 — Vercel 쪽에 별도로 같은 비밀을 복사해둘 필요가 없다(비밀이 존재하는 곳이 하나 늘지 않는다).
- Supabase Edge Function은 `verify_jwt` 설정만으로 **요청의 Authorization 헤더가 유효한 Supabase JWT인지를 함수 코드 실행 전에 플랫폼이 먼저 검증**해준다 — 인증 안 된 요청은 함수 코드에 도달하지도 못하고 자동으로 거부된다. Vercel Function으로 만들면 이 검증을 직접 구현해야 한다.
- 클라이언트에서 호출도 `supabase.functions.invoke(...)`로 통일돼 있어, 별도 API 클라이언트/CORS 설정이 필요 없다.

**함수 설계 스케치**(코드는 다음 단계):

```ts
// supabase/functions/delete-account/index.ts (설계 스케치)
import { createClient } from 'jsr:@supabase/supabase-js@2';

Deno.serve(async (req) => {
  // verify_jwt=true(config.toml)이므로 여기 도달했다는 것 자체가 유효한 Supabase JWT를 가진
  // 요청이라는 뜻이다. 그래도 "누구인지"는 요청 본문이 아니라 그 JWT에서 서버가 직접 확인한다.
  ...
  if (getUserError || !user) {
    return new Response(JSON.stringify({ error: '인증 정보를 확인할 수 없습니다.' }), { status: 401 });
  }

  // service_role은 이 환경변수에만 존재한다 (Supabase가 Edge Function에 자동 주입, 별도 설정 불필요)
  const adminClient = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);

  // getUser()가 검증한 "요청자 본인의" id만 사용한다 — 요청 본문에 담긴 값은 절대 신뢰하지 않는다
  // (본문에 { userId } 같은 걸 받아서 그걸로 삭제하면, 임의의 남의 계정을 삭제시킬 수 있는 취약점이 된다).
...
핵심 보안 포인트 두 가지(§6.5.5와 연결):
1. `service_role`을 쓰는 클라이언트(`adminClient`)는 함수 안에서만 만들어지고, 함수 밖(브라우저)으로는 절대 나가지 않는다.
2. 삭제 대상 `user.id`는 **요청을 보낸 클라이언트가 무엇을 주장하든 상관없이**, 서버가 JWT로부터 직접 확인한 값만 쓴다 — 클라이언트는 "나를 삭제해줘"라고만 말할 수 있고, "누구를 삭제할지"는 클라이언트가 결정할 수 없다.

**데이터 정리(`applications` 테이블)**: `applications.user_id`가 `auth.users(id)`를 참조하는 FK다(`docs/SUPABASE.md` §2). `auth.users` 행을 삭제했을 때 연관된 `applications` 행이 자동으로 함께 삭제(`ON DELETE CASCADE`)되는지는 **실제 적용된 스키마에서 확인이 필요하다**(설계 문서에 FK만 명시돼 있고 `ON DELETE` 동작은 명시되지 않았다 — §5 체크리스트에 추가):
- `CASCADE`가 걸려 있으면: `deleteUser` 한 번으로 끝난다. 가장 단순하고 원자적(atomic)이라 이 방식을 권장한다 — 마이그레이션으로 `ON DELETE CASCADE`를 추가하는 것을 1차 제안으로 삼는다.
- `CASCADE`가 아니면(예: 기본값 `NO ACTION`): `auth.users` 삭제 자체가 FK 위반으로 실패할 수 있다. 이 경우 Edge Function이 `adminClient.auth.admin.deleteUser` 호출 **전에** 같은 `adminClient`(또는 RLS를 통과하는 `callerClient`도 가능 — 어차피 본인 행만 지우므로)로 `applications` 테이블에서 `user_id = user.id`인 행을 먼저 삭제한 뒤 유저를 삭제하는 2단계 처리가 필요하다. 두 단계가 원자적이지 않다는 점(중간에 실패하면 지원 데이터만 지워지고 계정은 남는 등)은 알려진 한계로 남긴다.

**탈퇴 확인 UX — 확정 제안: "확인 문구 재입력".** 기존 지원 카드 삭제는 2단계 확인 버튼(한 번 클릭 → "정말 삭제할까요?" → 한 번 더 클릭)만으로 충분했지만(`EditApplicationForm`의 `isConfirmingDelete` 패턴), 계정 탈퇴는 되돌릴 수 없고 사용자의 전체 이력을 지우는 훨씬 무거운 작업이라 그 정도로는 부족하다고 판단한다. 계정 이메일(또는 "탈퇴" 같은 고정 문구)을 입력창에 그대로 입력해야만 탈퇴 버튼이 활성화되는 방식을 제안한다. 화면에는 "이 작업은 되돌릴 수 없으며, 모든 지원 내역이 함께 삭제됩니다" 같은 경고 문구를 명확히 둔다.

**완료 후 처리**: Edge Function이 `{ ok: true }`를 반환하면, 클라이언트는 (a) 로컬 지원 목록 초기화(`resetApplications`, 기존 로그아웃 처리와 동일하게 재사용), (b) `signOut()` 호출 — 이미 서버에서 계정이 삭제된 뒤라 `signOut()`의 서버 측 무효화 호출 자체가 에러를 반환할 수 있지만, 그 결과와 무관하게 로컬 세션/스토어는 확실히 정리한다(§6.5.7 엣지케이스 2), (c) `/login`으로 이동하며 "탈퇴가 완료됐습니다" 안내 — `?passwordUpdated=1`과 같은 패턴으로 `?accountDeleted=1` 같은 쿼리 신호를 재사용할 수 있다.

#### 6.5.5 보안 원칙 (재확인)

- `service_role` 키는 **Edge Function 환경변수에만** 존재한다. 클라이언트 코드, `.env.local`(웹앱 쪽), git 저장소 어디에도 들어가서는 안 된다 — 이 프로젝트가 지금까지 지켜온 원칙(§1, `NEXT_PUBLIC_SUPABASE_ANON_KEY`만 클라이언트에 둔다)의 연장선이다.
- 탈퇴 Edge Function은 **요청자 본인만** 자기 계정을 삭제할 수 있어야 한다 — `verify_jwt`로 인증 자체를 강제하고, 삭제 대상 id는 반드시 검증된 JWT에서 얻은 값만 쓴다(요청 본문의 `userId` 같은 클라이언트 제공 값은 신뢰하지 않는다). 이 두 가지가 §6.5.4 함수 설계의 핵심이다.
- 비밀번호 변경 전 현재 비밀번호 재확인(§6.5.2)도 큰 틀에서 같은 원칙이다 — "유효한 세션이 있다"는 것과 "지금 이 요청을 보낸 사람이 정말 계정 소유자다"는 것은 완전히 같지 않을 수 있다.

#### 6.5.6 라우팅/접근

| 경로 | 접근 조건 | 재인증(비밀번호 재확인 등) 필요 여부 |
| --- | --- | --- |
| `/account` | 로그인 필수(`AuthGuard`) | 페이지 진입 자체는 필요 없음 |
| 비밀번호 변경(섹션) | 로그인 필수 | **필요** — 현재 비밀번호 재확인(이메일 identity 있는 계정만, §6.5.2) |
| 이메일 변경(섹션) | 로그인 필수 | 불필요(대신 새/기존 이메일 확인 메일로 검증) |
| 회원 탈퇴(섹션) | 로그인 필수 | 재인증 대신 확인 문구 재입력으로 강한 의도 확인(§6.5.4) |
| 탈퇴 Edge Function | Supabase JWT 필수(`verify_jwt`) | 함수 자체가 JWT로 본인 확인(§6.5.4) |

#### 6.5.7 엣지 케이스

1. **이메일 변경 중 이탈**: 새 이메일 제출 후 확인 링크를 클릭하지 않고 이탈해도 아무 것도 깨지지 않는다 — 기존 이메일이 계속 유효한 상태로 남고, 변경은 무기한 "대기 중"으로 남는다(또는 Supabase 쪽 링크 만료 정책에 따라 자연히 소멸). 사용자가 나중에 다시 `/account`에 와서 새 이메일을 다시 제출하면 새 요청으로 덮어써진다고 가정한다(정확한 재시도 동작은 착수 시 확인).
2. **탈퇴 후 잔여 세션**: Edge Function이 성공적으로 계정을 지운 뒤에도, 클라이언트가 들고 있던 access token은 만료 전까지 형태상 "유효한 JWT"로 남아있을 수 있다(서명 자체는 계정 삭제와 무관하게 유효). §6.5.4의 "완료 후 처리"에서 클라이언트가 스스로 로그아웃 처리를 하는 이유가 이것이다 — 서버가 즉시 그 토큰을 무효화해주길 기다리지 않고, 클라이언트가 능동적으로 세션을 지운다. 만약 이 처리 없이 페이지를 닫아버리면, 다음에 그 브라우저가 만료 전 토큰으로 API를 호출할 때 `user_id`가 더 이상 존재하지 않아 RLS 조건(`auth.uid() = user_id`)에 걸리는 행이 하나도 없게 되어 사실상 빈 데이터로 보이거나 개별 요청이 실패하는 정도로 그친다 — 심각한 보안 문제로 이어지지는 않지만 UX가 지저분해지므로 명시적 로그아웃 처리를 권장한다.
3. **소셜 로그인(구글) 계정의 비밀번호 변경**: 이메일/비밀번호로 가입한 적 없이 구글로만 가입한 계정은 확인할 "현재 비밀번호" 자체가 없다. `user.identities`에 `provider === 'password'`(또는 동등한) 항목이 있는지로 판별해, 없으면 §6.5.2의 재확인 단계를 건너뛰고 화면 문구도 "비밀번호 변경" 대신 "비밀번호 설정"으로 바꾼다(구글 계정에 처음으로 이메일+비밀번호 로그인 수단을 추가하는 것이므로). Phase 4(소셜 로그인) 착수 후에만 실제로 발생하는 케이스이므로, Phase 4가 Phase 5보다 먼저 끝나 있어야 이 분기를 제대로 테스트할 수 있다.
4. **탈퇴 확인 문구를 틀리게 반복 입력**: 단순 UX 문제라 별도 설계는 필요 없다 — 버튼을 계속 비활성 상태로 두면 된다.

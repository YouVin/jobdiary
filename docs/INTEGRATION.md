...
1. `externalId`로 기존 저장분에 같은 값이 있는지 확인한다.
   - 있으면 스킵한다 (기술적 중복, 여기서 종료).
2. `externalId`가 없거나 새 값이면 `company` + `position`이 같은 기존 지원건이 있는지 확인한다.
   - 있으면: 추가하되 "중복 지원"으로 감지한다.
   - 없으면: 일반 추가한다.

원티드는 `externalId`가 없으므로 1)을 건너뛰고 `company` + `position` + `appliedAt` 조합으로 유사 판별한다.

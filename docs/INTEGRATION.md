...
- **"중복 지원" 표시는 실시간 계산 방식으로 확정.** `add-duplicate` 감지 결과를 DB에 별도 필드로 저장하지 않는다. 보드를 렌더링하는 시점에 현재 목록을 `company`+`position`으로 묶어, 서로 다른 `platform`이 2개 이상 섞인 그룹에 속한 건들을 "중복 지원"으로 표시한다(`src/utils/duplicateDetection.ts`의 `getDuplicateApplicationIds`, 카드 컴포넌트에서 `useMemo`로 목록이 바뀔 때만 재계산).
  - 저장하지 않고 매번 계산해도 되는 이유: 판정 기준(`company`+`position`+`platform`)이 이미 저장된 필드만으로 계산 가능하고, 지원 건수가 많지 않아 렌더 시 계산 비용이 무시할 만하다.
  - 같은 `platform`끼리는 대상에서 제외한다 — 같은 공고의 상태 변화는 이미 §5 판별 순서의 2번(`update`)에서 합쳐지므로, 여기서 또 중복으로 잡을 대상이 아니다.
...
- ~~"중복 지원" 표시 UI (카드 뱃지 등)는 데이터 갖춘 뒤 별도 설계~~ → 완료. 저장 없이 렌더 시 실시간 계산하는 방식으로 구현됨 (§5 "플래그·업데이트 저장 방식" 참조).
